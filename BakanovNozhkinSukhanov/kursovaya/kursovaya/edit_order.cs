...
namespace kursovaya
{
    ...
    {
        public edit_order(string str, DateTime d2)
        {
            InitializeComponent();
            tmpboxname = str;
            tmpd2 = d2;
        ...
        private void Init()
        {
            this.Text = "Редактирование записи об аренде - " + tmpboxname;
            ...
            order = orderlist.FindCLass(tmpboxname, tmpd2);
            if (order.box_owner_category == "Клиенты")
            ...
            if (comboBox1.SelectedIndex == 0)
            {
                ...
                if (File.Exists("clients.xml"))
                {
                    ...
                    //
                    obj = clientlist.FindCLass2(order.box_owner_login);
                    comboBox2.Text = obj.login;
                    ...
                    dateTimePicker1.Value = order.startdate;
            ...
            if (comboBox1.SelectedIndex == 1)
            {
                ...
                rcl = rclientlist.FindCLass(order.box_owner_login, order.box_owner_lastname);

                textBox1.Text = rcl.r_lastname;
                textBox2.Text = rcl.r_name;
                textBox3.Text = rcl.r_name_2;
                richTextBox1.Text = rcl.kontakt;
                textBox4.Text = rcl.auto_model;
                textBox5.Text = rcl.auto_number;
        ...
        private bool check_date(string str, DateTime dat1, DateTime dat2)
        {
            if (File.Exists("orders.xml"))
            {
                ...
                order2 = orderlist.FindCLass(tmpboxname, tmpd2);

                for (int i = 0; i < orderlist.coun(); i++)
                {
                    order = orderlist.ReturnMyClass(i);
                    if (order.Equals(order2) == false)
                    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            "Ошибка регистрации", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        ...
                        {
                            DialogResult dialogResult = MessageBox.Show("Сохранить изменения?", "Сохранение", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
                            if (dialogResult == DialogResult.Yes)
                            {
                                ...
                                orderlist.RemoveMyClass(tmpboxname, tmpd2);
